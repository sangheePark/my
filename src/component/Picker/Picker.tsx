...
class Picker extends React.Component<IPickerProp & IPickerProps, any> {
  ...
  scrollHanders = (() => {
    ...
    const onMove = (y: number) => {
      ...
      if (this.props.isValid) {
        ...
        if (
          child.props &&
          !this.props.isValid(selectedValue, child.props.value)
        ) {
          return;
        }
      }
  ...
  componentDidMount() {
    ...
    (rootRef as HTMLDivElement).addEventListener(
      "mousedown",
      this.scrollHanders.mousedown,
      willNotPreventDefault
    );
    (rootRef as HTMLDivElement).addEventListener(
      "mousemove",
      this.scrollHanders.mousemove,
      willPreventDefault
    );
    (rootRef as HTMLDivElement).addEventListener(
      "mouseup",
      this.scrollHanders.mouseup,
      willNotPreventDefault
    );
  }
