...
type IProps = {
	...
	onClose?: (eventType: any) => void //닫기 1순위 함수
...
const Page: React.FC<IProps> = ({
	...
	const handleScroll = () => {
		const scrollHeight = document.getElementsByClassName('content-area')[0].scrollHeight
		const clientHeight = document.getElementsByClassName('content-area')[0].clientHeight
		const scrollTop = Math.ceil(document.getElementsByClassName('content-area')[0].scrollTop)
		const scrollLeft = Math.ceil(document.getElementsByClassName('content-area')[0].scrollLeft)
		console.log(
	...
	return (
		<div className={cx('block full-width relative detail', className)} onClick={onClick}>
